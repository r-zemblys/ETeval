...
def get_arguments():
    ...
    parser.add_argument('--exp', type=str, default=None, help='Experiment')
...
columns = ['dataset', 'algorithm', 'fname', 'ke_fix', 'ke_sacc', 'ke_pso']
...
for _data in data:
    ...
    fpath_rez = '%s/%s/%s_%s.csv' % (_data['root'], _data['pr'], _data['alg'],
                                     _suffix)
    print(fpath_rez)
    fpath_log = '%s/%s/%s-%s.log' % (
        _data['root'], _data['pr'], _data['alg'],
    ...
    for fpath in tqdm(sorted(FILES)):
        ...
        _etdata_gt.data['evt'] = class_map_func(_etdata_gt.data['evt'],
                                                class_mapper)
        _etdata_pr.data['evt'] = class_map_func(_etdata_pr.data['evt'],
                                                class_mapper)

        # evaluate per trial
        ke, (evt_overlap, _evt_gt, _evt_pr) = eval_evt(_etdata_gt,
                                                       _etdata_pr,
                                                       len(classes),
                                                       mode=eval_mode)
    ...
    etdata_gt.load(etdata_gt_meta, **{'source': 'array'})
    etdata_pr.load(etdata_pr_meta, **{'source': 'array'})
    ke, (evt_overlap, _evt_gt, _evt_pr) = eval_evt(etdata_gt,
                                                   etdata_pr,
                                                   len(classes),
                                                   mode=eval_mode)
